...
static int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)
{
  /* USER CODE BEGIN 6 */
	...
	if(s_RxBuffers.pos_receive >= MaxCommandsInBuffer) s_RxBuffers.pos_receive=0;
...
{
	if (s_RxBuffers.IsCommandDataReceived == 0) return 0;
	...
	*Len = s_RxBuffers.CommandsLens[index];
	...
	if (s_RxBuffers.pos_process >= MaxCommandsInBuffer) s_RxBuffers.pos_process=0;
